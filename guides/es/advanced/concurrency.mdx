...
<Note>
  Los valores de nombres de métodos en estos ejemplos se comparan sin distinguir
  entre mayúsculas y minúsculas, por lo que `worldwide`, `Worldwide` y
  `WORLDWIDE` se tratan igual.
</Note>
...
La coincidencia de nombres de métodos no distingue entre mayúsculas y minúsculas en ambos lugares, así que usa el mismo texto de nombre de método aunque las mayúsculas no importen.
