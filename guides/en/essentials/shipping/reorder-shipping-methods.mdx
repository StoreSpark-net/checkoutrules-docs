...
To configure the rule, first optionally we can add a name such as "Reorder shipping methods" to describe what it does. Then, select the rule [status](/guides/get-started/concepts#status): Testing, Active, or Deactivated.
...
Next, on the same screen, select the appropriate [Target](/guides/get-started/concepts#target) to define where the rule should apply. We can choose _"Ship"_ to reorder shipping methods shown after a **shipping address** is entered. Then, we can select:
