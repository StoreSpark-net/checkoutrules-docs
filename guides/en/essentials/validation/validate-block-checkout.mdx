...
<iframe
  ...
  title="BeSure Validate/Block Checkout"
  style={{ border: 0 }}
...
Next, on the same screen, select how the system should behave if the app fails or experiences a runtime problem during checkout. In most cases, the safe default is to allow customers to complete checkout. You’ll find two options:

- **Allow customer to complete checkout**: even if the app encounters an issue, the customer can still proceed with purchase.

- **Block customer from completing checkout**: checkout is blocked if the app fails.
...
<Note>
  This setting is different from rule-level **Date Time / GeoIP fallback**:

  - This page's setting controls what happens if the app itself fails at runtime.
  - Date Time / GeoIP fallback controls what action to run when the app cannot
    determine time/date or customer location context for conditions that use
    those values.
</Note>
