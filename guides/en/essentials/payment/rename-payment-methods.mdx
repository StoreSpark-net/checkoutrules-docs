...
Method-name matching is case-insensitive, so `cash on delivery`, `Cash on Delivery`, and `CASH ON DELIVERY` are treated the same. Exact matching still requires the full method name; capitalization does not matter.
