...
- **Fixed ranking** – This option lets you manually set the order of payment methods by assigning a fixed rank (e.g., 1, 2, 3, etc.) to each method, so you can control exactly which method appears in which **position**. Method-name matching is case-insensitive, so `visa`, `Visa`, and `VISA` are treated the same.
