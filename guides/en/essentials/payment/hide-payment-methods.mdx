...
- **Hide these payment methods** – Hides only the payment methods you specify when the condition matches. Method-name matching is case-insensitive, so `visa`, `Visa`, and `VISA` are treated the same.
...
- **Only show these payment methods** – Shows only the specified payment methods when the condition matches and hides all others. Method-name matching is case-insensitive, so `visa`, `Visa`, and `VISA` are treated the same.
