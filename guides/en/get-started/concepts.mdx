...
Strategy controls how matching discount blocks are applied:

- **Order discount strategy**: First or Maximum.
- **Product discount strategy**: First, All, or Maximum.
- **Shipping discount behavior**: First.
- When both **Order** and **Product** discounts are enabled in Discount all, you
  can either:
  - keep one shared strategy using **Apply the same strategy to product and
    order discounts**
  - or configure order and product strategies separately.

Used in current discount flows (including **Discount all** and order/product discount configurations).
...
Defines what should happen if the app cannot determine **time/date** or **customer location** at checkout for rules using those conditions.

- **Do nothing**: no action is applied by the rule in that case.
- **Do this action**: the fallback action you configure is applied instead.

Used in rules that support **Date Time** or **Customer location (using GeoIP)** conditions.

<img src="/images/concepts/fallback.webp" alt="Fallback" />

This fallback is different from the **Validation block-on-failure** setting:

- **Fallback (this section)**: what action to run when Date Time / GeoIP context is unavailable.
- **Validation block-on-failure**: whether checkout is allowed or blocked if the app experiences a runtime issue.
