...
<Accordion title="Can I use wildcard like * to select shipping or payment methods?">
  ...
  match with a single character. Wildcard method-name matching is also
  case-insensitive, so `standard*`, `Standard*`, and `STANDARD*` are treated the
  same.
</Accordion>
...
<Accordion title="What is the difference between 'is' and 'contains' used in different rules?">
  ...
  Shipping' or 'Standard Delivery'. Method-name matching is case-insensitive, so
  `visa`, `Visa`, and `VISA` are treated the same. Exact matching still requires
  the full method name; capitalization does not matter.
</Accordion>
