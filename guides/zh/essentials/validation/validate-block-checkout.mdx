...
然后，在 **模板选择** 中，根据您的需求选择一个选项。在大多数情况下，您可以选择空白规则作为起点并根据需要进行修改。即使您选择了预填模板，也可以稍后修改。
...
要配置规则，首先我们可以选择性地添加一个名称，例如 "验证/阻止结账"，以描述其作用。然后，选择规则的 [状态](/guides/zh/get-started/concepts#地位)：测试中、激活或停用。
...
接下来，在同一界面下，为规则选择适当的 [行为](/guides/zh/get-started/concepts#行为)。您可以选择 **阻止结账并显示错误消息**、**显示警告消息而不阻止结账**，或 **仅在客户尝试完成订单时阻止结账**，具体取决于您希望规则如何影响结账流程。
...
> 例如，这里我们使用默认的 **条件集 1**，设置规则在 **"地址行 2（公寓、套房等）"** 字段为空时触发，并配置其阻止结账。错误消息 **"请输入完整地址"** 将显示在结账页面的 **配送地址行 2** 字段下，要求客户提供完整地址后才能继续。
...
- 在 **以下输入字段下方** 分类下，您可以选择将操作显示在特定输入字段下方，例如：电子邮件、客户电话号码、配送国家/地区、配送名字、配送姓氏、配送公司、配送地址（地址行 1）、配送公寓、套房等（地址行 2）、配送城市、配送省/州、配送邮政编码、配送电话号码、账单国家/地区、账单名字、账单姓氏、账单公司、账单地址（地址行 1）、账单公寓、套房等（地址行 2）、账单城市、账单省/州、账单邮政编码和账单电话号码。
...
之后，您可以在 **"错误消息"** 字段中输入您的消息。您输入的消息将显示在结账页面上。
...
您还可以通过点击 **"添加翻译"** 按钮并为所需语言输入翻译后的消息来添加翻译（可选）。在添加翻译之前输入的消息将用作 **默认消息**，并在没有可用翻译时显示。
...
在将地址行 2（公寓、套房等）字段留空后，结账规则运行并阻止了结账流程。我们可以验证错误消息 **"请输入完整地址"** 已显示在结账页面的 **配送地址行 2** 字段下，如下所示。
