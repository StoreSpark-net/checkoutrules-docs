...
<Note>
  Les valeurs de noms de méthodes dans ces exemples sont comparées sans tenir
  compte de la casse, donc `worldwide`, `Worldwide` et `WORLDWIDE` sont traités
  de la même façon.
</Note>
...
La correspondance des noms de méthodes est insensible à la casse aux deux endroits, donc utilisez le même texte de nom de méthode même si la casse n’a pas d’importance.
