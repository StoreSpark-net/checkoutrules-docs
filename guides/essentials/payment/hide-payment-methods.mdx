...
description: 'Hide certain payment methods at the checkout using given conditions.'
icon: 'eye-slash'
---

## Rule overview

This rule allows you to automatically hide specific payment methods at checkout based on defined conditions. You can control visibility using criteria such as cart total, billing address, customer tags, or product details.
...
Then, You’ll be redirected to the **Select checkout rule type screen**. Choose _category "Payment" → select a checkout rule type "Hide Payment Methods" →  click “Select Rule”_.
...
### Rule configuration
To configure the rule, first optionally we can add a name such as "Hide payment methods" to describe what it does. Then, select the [status](/guides/get-started/concepts#status): Testing, Active, or Deactivated. 
