...
This rule allows you to validate or block the checkout process based on defined conditions. You can show custom error messages or completely prevent checkout when certain criteria are not met, such as invalid cart contents, restricted shipping or billing regions, customer tags, or missing product attributes.

This helps enforce business rules, prevent unsupported orders, and guide customers to resolve issues before completing checkout.
...
Then, You’ll be redirected to the **Select checkout rule type screen**. Choose _category "Validation " → select a checkout rule type "Validate/Block Checkout" →  click “Select Rule”_.
...
### Rule configuration
To configure the rule, first optionally we can add a name such as "Validate/block checkout" to describe what it does. Then, select the [status](/guides/get-started/concepts#status): Testing, Active, or Deactivated. 
...
> As an example, here we are using the default **Condition set 1**, setting the rule to trigger when the **"Address line 2 (Apartment, suite, etc)"** field is **empty**, and configuring it to block checkout. The error message **"Enter complete address"** will be displayed in the **Address line 2** field at checkout, requiring customers to provide their complete address before proceeding.
...
In the **Action** section, you can choose the **target location** where your custom action will appear on the checkout page. The dropdown menu provides a list of placement options:
...
This gives you control over where the action appears, helping you place it in the most relevant section of the checkout form.
...
Next, on the same screen, select how the system should behave if the app fails or experiences a problem during checkout. You’ll find two options:
...
After leaving the Address line 2 (Apartment, suite, etc) field **empty**, the checkout rule runs and blocks the checkout process. We can verify that the error message **"Enter complete address"** is displayed in the **Address line 2** field at checkout, as shown below.
<img
