...
要配置规则，首先我们可以选择性地添加一个名称，例如 "验证/阻止结账"，以描述其作用。然后，选择规则的 [状态](/guides/zh-hans/get-started/concepts#status)：测试中、激活或停用。
...
接下来，在同一界面下，至少选择一个 [触发此规则](/guides/zh-hans/get-started/concepts#trigger) 选项。您可以选择 **结账时交互**（在客户进行结账时运行规则），以及 **完成结账**（在客户完成结账时运行规则）。默认情况下，您应该同时选择两个选项，以确保规则在需要时始终运行。
