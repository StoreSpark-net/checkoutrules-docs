...
For **Has any of these values**, **Does not have any of these values**, **Has any other value besides these values**, or **Does not have any other value besides these values**, enter one or more values to match. Separate multiple values with commas. Method-name matching is case-insensitive, so `express shipping`, `Express Shipping`, and `EXPRESS SHIPPING` are treated the same.

<Note>
  Use * wildcard to match partial shipping method names. For example, Stand*
  will match Standard, standard shipping etc. Wildcard method-name matching is
