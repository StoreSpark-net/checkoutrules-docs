...
- **shipping name contains** (Matches when the shipping method name includes the entered text as partial text)
- **shipping name is** (Matches exact text, or wildcard pattern if `*` is used)
...
<Note>
  Shipping-name matching is case-insensitive. You can use `*` wildcard in input
